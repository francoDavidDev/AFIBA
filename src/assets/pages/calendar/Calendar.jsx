...
const Calendar = () => {
  const TOURNAMENTS_2024 = [
    { name: "Sanson Cup", date: "2024-02-24", location: "Palacio de deportes Mar del Plata", info: '2266440219', hour: '10:00 hs', image: image5, status: 'terminado', registration: '#', inscriptions: false, available: false },
    { name: "Campeonato Apertura", date: "2024-05-12", location: "Colon 600 - MERLO", info: '2204081490', hour: '14:00 hs', image: image2, status: 'terminado', registration: '#', inscriptions: false, available: false },
    { name: "Copa independencia", date: "2024-07-07", location: "San Andres de Giles Bs.As", hour: '14:00 hs', info: '+54 92325 47-9243', image: image1, status: 'terminado', registration: '#', inscriptions: false, available: false },
    { name: "Copa Provincia", date: "2024-07-21", location: "Arieta 2917 - San Justo - Bs.As", info: '1127796576 o 1125193782', hour: '11:00 hs', image: image4, status: 'terminado', registration: '#', inscriptions: false, available: false },
    { name: "Campeonato Bonaerense", date: "2024-10-13", location: "La Plata Bs.As", info: '+54 92281 58-9117', hour: '09:00 hs', image: image3, status: 'presente', registration: '#', inscriptions: true, available: true },
    { name: "Campeonato Clausura", date: "2024-12-15", location: "Gregorio De La Ferrere", info: '+54 9 1128729216', hour: '--', image: image6, status: 'presente', registration: '#', inscriptions: false, available: false }
  ];
    ...
    <div className="container mx-auto py-12 px-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {filteredTournaments.map((tournament, index) => (
          <motion.div
            key={index}
            className={`rounded-lg overflow-hidden shadow-lg transition-transform transform hover:scale-105 ${!tournament.available ? 'opacity-50 cursor-not-allowed' : ''}`}
            whileTap={{ scale: 0.95 }}
          >
            <motion.div className="relative">
              ...
              {!tournament.inscriptions && !tournament.available && (
            ...
            >
              ...
              <button className="bg-primary-500 text-white font-bold py-2 px-4 rounded mt-4" onClick={() => handleCardClick(tournament)}>
              <Link to={'/registrationForm'} >Preinscripcion</Link>
                </button>
              <button className="bg-primary-500 text-white font-bold py-2 px-4 rounded mt-4" onClick={() => handleCardClick(tournament)} > <Link to={'/EventInfo'}>Más información</Link></button>
            </motion.div>
          </motion.div>
        ))}
      </div>
